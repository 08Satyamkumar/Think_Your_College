...
function FloatingInput({
  ...
  return (
    <div className="relative group">
      <div
        className={`flex items-center w-full rounded-xl border-2 transition-all duration-300 overflow-visible ${
          focused
      ...
      >
        <div className="relative flex-1 h-12">
          <Icon
            className={`absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors duration-200 pointer-events-none z-10 ${
...
function PhoneInput({
  ...
  return (
    <div
      className={`flex items-center w-full rounded-xl border-2 h-12 transition-all duration-300 overflow-hidden ${
        focused
          ? "border-indigo-500 bg-indigo-50/40 shadow-[0_0_0_3px_rgba(99,102,241,0.18)]"
          : "border-slate-200 bg-white hover:border-indigo-300"
      }`}
    >
      ...
      <div className="relative flex-1 h-full flex items-center">
        <Phone
          className={`absolute left-3 w-4 h-4 pointer-events-none transition-colors ${focused ? "text-indigo-500" : "text-slate-400"}`}
        />
        <input
          type="tel"
          value={value}
...
export default function AuthModal({
    ...
    try {
      await fetch("/api/leads", {
        ...
        body: JSON.stringify({
          name: name || "Student",
          email: email || null,
          phone: phone || `${source} Auth`,
          course_interest: `Student Registered (${source})`,
          status: "Registered User",
        }),
      });
    } catch (e) {
      console.error("Failed to store auth lead data:", e);
    }
  };
  useEffect(() => {
    if (isOpen) {
  ...
  const handleGoogleLogin = () => {
    ...
    const activeClientId =
      envClientId ||
      "102650280321-egj6rje44f90klbgi0a5l7t1fb6p62um.apps.googleusercontent.com";
    if (
      typeof window === "undefined" ||
  ...
  const handleSendOtp = () => {
    const valid =
      loginType === "mobile" ? phone.length === 10 : email.includes("@");
    if (valid) {
      setOtpSent(true);
      setOtpTimer(30);
    }
                      ...
                      >
                        {features.map((f, i) => (
                          ...
                          >
                            ...
                            <span className="text-[9px] font-bold text-slate-400 text-center leading-tight max-w-[58px]">
                              {f.text}
                            </span>
                          </motion.div>
                        ))}
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 text-[11px] text-slate-500 bg-indigo-50/70 rounded-xl px-3 py-2 border border-indigo-100 ">
                          ...
                          <span className="flex-1">
                            Greater Noida is your current location.
                          </span>
                          <button className="font-black text-indigo-500 hover:text-indigo-700 transition-colors">
                            Change
                          </button>
                        </div>
                        {/* Terms */}
                        ...
                        <motion.button
                          ...
                          onClick={() => {
                            const nameVal = fullName || "Student";
                            const emailVal =
                              signupEmail || `${signupPhone}@tyc.com`;
                            saveLeadData(
                              nameVal,
                              emailVal,
                              signupPhone,
                            ...
                            onLoginSuccess({ name: nameVal, email: emailVal });
                          }}
                          className="w-full py-3 rounded-xl bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 disabled:from-slate-200 disabled:to-slate-200 disabled:cursor-not-allowed text-white disabled:text-slate-400 font-black text-sm shadow-md shadow-orange-500/25 transition-all duration-300 flex items-center justify-center gap-2"
                        >
